...
namespace GMapping {
  ...
  bool GridSlamProcessor::processScan(const RangeReading & reading, int adaptParticles)
  {
    ...
    {
      //对于每一个粒子，将从运动模型传播的激光雷达位姿存放到m_particles[i].pose
    ...
    if (!m_count 
      ...
      if (m_count>0)
      {
        /*功能：为每一个粒子，在当前激光雷达位姿下，当前帧激光数据下，求解每个粒子的位姿最优值
            （策略：爬山，评价机制：得分机制（激光雷达在当前里程计位姿下激光数据和当前地图的的匹配程度））
                求解每个粒子的权重
        ...
        */
        ...
        scanMatch(plainReading);  // 这里没有直接使用激光雷达坐标系下的点云去匹配，而是将激光数据放到粒子pose上去匹配，体现gridfastslam的定位和建图分开的思路
